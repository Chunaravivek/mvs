            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Punjabi Tag Types</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Punjabi Tag Types</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('PunjabiTagTypes', array('url' => array('controller' => 'PunjabiTagTypes', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>            
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit Punjabi Tag Types</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        $('#design_type').select2({
            ...
            width: '100%',
            ajax: {
                url: '<?php echo URL_PATH;?>PunjabiTagTypes/DesignTypeSelect',
                dataType: 'json',
//                delay: 250,
                data: function (data) {
                    return {
        ...
        var url = "<?php echo URL_PATH;?>PunjabiTagTypes/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.PunjabiTagTypes.id + '>';
                    },
                },
                {mData: 'PunjabiTagTypes.id'},
                {mData: 'PunjabiTagTypes.name'},
                {mData: 'PunjabiTagTypes.design_type'},
                {mData: 'PunjabiTagTypes.type_text'},
                {mData: 'PunjabiTagTypes.order'},
                {mData: 'PunjabiTagTypes.status','class':'project-status'},
                {mData: 'PunjabiTagTypes.created_date'},
                {mData: 'PunjabiTagTypes.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View PunjabiTagTypes" onclick="ViewModal('+mData.PunjabiTagTypes.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit PunjabiTagTypes" onclick="EditModal('+mData.PunjabiTagTypes.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete PunjabiTagTypes" onclick="deleteModal('+mData.PunjabiTagTypes.id+')"><i class="ft-trash-2"></i></a>';
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                if (mData.PunjabiTagTypes.status == '1') {
                    $('td:eq(6)', nRow).html('<input type="checkbox" checked class="PunjabiTagTypes_status" id="'+mData.PunjabiTagTypes.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(6)', nRow).html('<input type="checkbox" class="PunjabiTagTypes_status" id="'+mData.PunjabiTagTypes.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
                $('.PunjabiTagTypes_status').checkboxpicker();
               
                $('.PunjabiTagTypes_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'PunjabiTagTypes/update_status',
                        data: {
        ...
        $('#form-field-select-1').on('change', function(e) {
            ...
            $('#DataTables_Table_0').DataTable().ajax.url("PunjabiTagTypes/records?design_type="+design_type).load();
        });
    });
    
    $(function () { 
        $('#PunjabiTagTypesAddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
                "data[PunjabiTagTypes][name]": {
                    required: true,
                },
                "data[PunjabiTagTypes][design_type]": {
                    required: true,
                },
                "data[PunjabiTagTypes][type_text]": {
                    required: true,
                },
                "data[PunjabiTagTypes][order]": {
                    required: true,
                },
            },
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>PunjabiTagTypes/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>PunjabiTagTypes/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>PunjabiTagTypes/deleteAll',
                        data: {
                            id : selected_values
                        },
