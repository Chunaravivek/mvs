...
?>
<?php
    echo $this->Form->create('PunjabiVideos', array('url' => '/PunjabiVideos/save/'.$id, 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); 
?>
            ...
            <div class="col-md-9 mx-auto">
                <div class="controls">
                    ...
                    if (isset($this->request->data['PunjabiVideos']['url']) && $this->request->data['PunjabiVideos']['url'] != '') {
                        ...
                        echo "<button class='ui-button ui-widget ui-corner-all ml-1' id='confirm-delete-images' title='" . $url . "' data-id='".$this->request->data['PunjabiVideos']['id']."'><span class='ui-icon ft-image'></span>Delete PunjabiVideos</button>";
                        echo "</p>";
                    } else {
...
//                delay: 250,
    ...
    $('#PunjabiVideosEditForm').validate({
        errorElement: 'div',
    ...
    $(document).on('click', '#confirm-delete-images', function (e) {
        ...
        Swal.fire({
            ...
            if (result.value == true) {
                $.ajax({
                    type: 'POST',
                    url: '<?php echo URL_PATH; ?>PunjabiVideos/delete_image',
                    data: {
                        'id': id,
                    },
                    success: function (msg) {
                        $('#removed-p-tag').remove();
                        $('#image_loc').find('.controls').append('<input type="file" name="data[PunjabiVideos][url]" error="error" id="url1" class="form-control file"">');
