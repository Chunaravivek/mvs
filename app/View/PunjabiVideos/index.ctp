            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Punjabi Videos</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Punjabi Videos</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('PunjabiVideos', array('url' => array('controller' => 'PunjabiVideos', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
...
<div class="modal animated bounceIn" id="modal-default-yt" role="dialog">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add YouTube PunjabiVideos </h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('PunjabiVideos', array('url' => array('controller' => 'PunjabiVideos', 'action' => 'ytadd'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
                <div class="modal-body">
                    <div class="form-body">
                        <div class="form-group row">
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit PunjabiVideos</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        $('#app_id').select2({
            ...
            width: '100%',
            ajax: {
                url: '<?php echo URL_PATH; ?>PunjabiVideos/AppSelect',
                dataType: 'json',
//                delay: 250,
                data: function (data) {
                    return {
        ...
        var url = "<?php echo URL_PATH;?>PunjabiVideos/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.PunjabiVideos.id + '>';
                    },
                },
                {mData: 'PunjabiVideos.id'},
                {mData: 'PunjabiVideos.title'},
                {mData: 'PunjabiVideos.tags'},
                {mData: 'PunjabiVideos.url'},
                {mData: 'PunjabiVideos.views'},
                {mData: 'PunjabiVideos.downloads'},
                {mData: 'PunjabiVideos.status','class':'project-status'},
                {mData: 'PunjabiVideos.created_date'},
                {mData: 'PunjabiVideos.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View PunjabiVideos" onclick="ViewModal('+mData.PunjabiVideos.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit PunjabiVideos" onclick="EditModal('+mData.PunjabiVideos.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete PunjabiVideos" onclick="deleteModal('+mData.PunjabiVideos.id+')"><i class="ft-trash-2"></i></a>';
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                var thumb = mData.PunjabiVideos.url  + '.jpg';
//                $('td:eq(4)', nRow).html('<video controls poster="' + thumb + '" style="height: 200px;width: 350px"><source src="' + mData.PunjabiVideos.url + '" type="video/mp4"></video>');
                $('td:eq(4)', nRow).html('<a class="ui-button ui-widget ui-corner-all mb-2" href="' + mData.PunjabiVideos.url + '" target="_blank"><span class="ui-icon ft-play"></span></a>');
                if (mData.PunjabiVideos.status == '1') {
                    $('td:eq(7)', nRow).html('<input type="checkbox" checked class="PunjabiVideos_status" id="'+mData.PunjabiVideos.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(7)', nRow).html('<input type="checkbox" class="PunjabiVideos_status" id="'+mData.PunjabiVideos.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
                $('.PunjabiVideos_status').checkboxpicker();
               
                $('.PunjabiVideos_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'PunjabiVideos/update_status',
                        data: {
        ...
        $('#form-field-select-1').on('change', function(e) {
            ...
            $('#DataTables_Table_0').DataTable().ajax.url("PunjabiVideos/records?tags_id="+tags_id).load();
        });
        
    });
    
    $(function () { 
        $('#PunjabiVideosAddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
                "data[PunjabiVideos][app_id]": {
                    required: true,
                },
                "data[PunjabiVideos][yturl]": {
                    required: true,
                    url: true,
                },
                "data[PunjabiVideos][title]": {
                    required: true,
                },
                "data[PunjabiVideos][url]": {
                    required: true,
                },
                "data[PunjabiVideos][tags_id]": {
//                "data[PunjabiVideos][tags_id][]": {
                    required: true,
                },
                "data[PunjabiVideos][views]": {
                    required: true,
                },
                "data[PunjabiVideos][downloads]": {
                    required: true,
                },
                "data[PunjabiVideos][modified_date]": {
                    required: true,
                },
            },
        ...
        $('#PunjabiVideosYtaddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>PunjabiVideos/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>PunjabiVideos/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>PunjabiVideos/deleteAll',
                        data: {
                            id : selected_values
                        },
