            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage God Videos</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add GodVideos</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('GodVideos', array('url' => array('controller' => 'GodVideos', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
...
<div class="modal animated bounceIn" id="modal-default-yt" role="dialog">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add YouTube GodVideos </h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('GodVideos', array('url' => array('controller' => 'GodVideos', 'action' => 'ytadd'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
                <div class="modal-body">
                    <div class="form-body">
                        <div class="form-group row">
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit GodVideos</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        $('#app_id').select2({
            ...
            width: '100%',
            ajax: {
                url: '<?php echo URL_PATH; ?>GodVideos/AppSelect',
                dataType: 'json',
//                delay: 250,
                data: function (data) {
                    return {
        ...
        var url = "<?php echo URL_PATH;?>GodVideos/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.GodVideos.id + '>';
                    },
                },
                {mData: 'GodVideos.id'},
                {mData: 'GodVideos.title'},
                {mData: 'GodVideos.tags'},
                {mData: 'GodVideos.url'},
                {mData: 'GodVideos.views'},
                {mData: 'GodVideos.downloads'},
                {mData: 'GodVideos.status','class':'project-status'},
                {mData: 'GodVideos.created_date'},
                {mData: 'GodVideos.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View GodVideos" onclick="ViewModal('+mData.GodVideos.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit GodVideos" onclick="EditModal('+mData.GodVideos.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete GodVideos" onclick="deleteModal('+mData.GodVideos.id+')"><i class="ft-trash-2"></i></a>';
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                var thumb = mData.GodVideos.url  + '.jpg';
//                $('td:eq(4)', nRow).html('<video controls poster="' + thumb + '" style="height: 200px;width: 350px"><source src="' + mData.GodVideos.url + '" type="video/mp4"></video>');
                $('td:eq(4)', nRow).html('<a class="ui-button ui-widget ui-corner-all mb-2" href="' + mData.GodVideos.url + '" target="_blank"><span class="ui-icon ft-play"></span></a>');
                if (mData.GodVideos.status == '1') {
                    $('td:eq(7)', nRow).html('<input type="checkbox" checked class="GodVideos_status" id="'+mData.GodVideos.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(7)', nRow).html('<input type="checkbox" class="GodVideos_status" id="'+mData.GodVideos.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
                $('.GodVideos_status').checkboxpicker();
               
                $('.GodVideos_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
        ...
        $('#form-field-select-1').on('change', function(e) {
            ...
            $('#DataTables_Table_0').DataTable().ajax.url("GodVideos/records?tags_id="+tags_id).load();
        });
        
    });
    
    $(function () { 
        $('#GodVideosAddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
                "data[GodVideos][app_id]": {
                    required: true,
                },
                "data[GodVideos][yturl]": {
                    required: true,
                    url: true,
                },
                "data[GodVideos][title]": {
                    required: true,
                },
                "data[GodVideos][url]": {
                    required: true,
                },
                "data[GodVideos][tags_id][]": {
                    required: true,
                },
                "data[GodVideos][views]": {
                    required: true,
                },
                "data[GodVideos][downloads]": {
                    required: true,
                },
                "data[GodVideos][modified_date]": {
                    required: true,
                },
            },
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>GodVideos/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>GodVideos/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>GodVideos/deleteAll',
                        data: {
                            id : selected_values
                        },
