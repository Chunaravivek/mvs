...
?>
<?php
    echo $this->Form->create('GodVideos', array('url' => '/GodVideos/save/'.$id, 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); 
?>
            ...
            <div class="col-md-9 mx-auto">
                <div class="controls">
                    ...
                    if (isset($this->request->data['GodVideos']['url']) && $this->request->data['GodVideos']['url'] != '') {
                        ...
                        echo "<button class='ui-button ui-widget ui-corner-all ml-1' id='confirm-delete-images' title='" . $url . "' data-id='".$this->request->data['GodVideos']['id']."'><span class='ui-icon ft-image'></span>Delete GodVideos</button>";
                        echo "</p>";
                    } else {
    ...
    $(document).on('click', '#confirm-delete-images', function (e) {
        ...
        Swal.fire({
            ...
            if (result.value == true) {
                $.ajax({
                    type: 'POST',
                    url: '<?php echo URL_PATH; ?>GodVideos/delete_image',
                    data: {
                        'id': id,
                    },
                    success: function (msg) {
                        $('#removed-p-tag').remove();
                        $('#image_loc').find('.controls').append('<input type="file" name="data[GodVideos][url]" error="error" id="url1" class="form-control file"">');
