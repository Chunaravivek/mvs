...
?>
<?php
    echo $this->Form->create('WishesTags', array('url' => '/WishesTags/save/'.$id, 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); 
?>
...
<!--        <div class="form-group row">
        ...
        <div class="form-group row" id="image_loc">
            ...
            <div class="col-md-9 mx-auto">
                <div class="controls">
                    <?php
                    if (isset($this->request->data['WishesTags']['images']) && $this->request->data['WishesTags']['images'] != '') {
                        echo "<p id='removed-p-tag'>";
                        echo "<img name='images' src=" .$this->request->data['WishesTags']['images'] . " height='60px' width='60px'>";
                        echo "<button class='ui-button ui-widget ui-corner-all ml-1' id='confirm-delete-images' title='" . $this->request->data['WishesTags']['images'] . "' data-id='".$this->request->data['WishesTags']['id']."'><span class='ui-icon ft-image'></span>Delete Images</button>";
                        echo "</p>";
                    } else {
            ...
            "data[WishesTags][name]": {
                ...
                remote: {
                    ...
                    data: {
                        ...
                        id : function() {
                            return <?php echo $this->request->data['WishesTags']['id']; ?>;
                        },
                    },
                    beforeSend: function() {
    ...
    $(document).on('click', '#confirm-delete-images', function (e) {
        ...
        Swal.fire({
            ...
            if (result.value == true) {
                $.ajax({
                    type: 'POST',
                    url: '<?php echo URL_PATH; ?>WishesTags/delete_image',
                    data: {
                        'id': id,
                    },
                    success: function (msg) {
                        $('#removed-p-tag').remove();
                        $('#image_loc').find('.controls').append('<input type="file" name="data[WishesTags][images]" error="error" id="images1" class="form-control file" aria-required="true">');
