            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Wishes Images</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Wishes Images</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('WishesImages', array('url' => array('controller' => 'WishesImages', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
                <div class="modal-body">
                    <div class="form-body">
                        ...
                        <div class="form-group row">
                            <label class="col-md-3 label-control" for="image"> Multiple WishesImages : </label>
                            <div class="col-md-9 mx-auto">
                                <div class="controls">
                                    <?php
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit Wishes Images</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        $('#app_id').select2({
            ...
            width: '100%',
            ajax: {
                url: '<?php echo URL_PATH; ?>WishesImages/AppSelect',
                dataType: 'json',
//                delay: 250,
                data: function (data) {
                    return {
        ...
        var url = "<?php echo URL_PATH;?>WishesImages/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.WishesImages.id + '>';
                    },
                },
                {mData: 'WishesImages.id'},
                {mData: 'WishesImages.title'},
                {mData: 'WishesImages.tags'},
                {mData: 'WishesImages.image'},
                {mData: 'WishesImages.views'},
                {mData: 'WishesImages.downloads'},
                {mData: 'WishesImages.status','class':'project-status'},
                {mData: 'WishesImages.created_date'},
                {mData: 'WishesImages.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View WishesImages" onclick="ViewModal('+mData.WishesImages.id+')"><i class="fas fa-info"></i></a>';
//                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit WishesImages" onclick="EditModal('+mData.WishesImages.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete WishesImages" onclick="deleteModal('+mData.WishesImages.id+')"><i class="ft-trash-2"></i></a>';
                        return    delete_id;
                    },
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                $('td:eq(4)', nRow).html('<img src="'+mData.WishesImages.image+'" class="media-object" width="64" height="64">');
                if (mData.WishesImages.status == '1') {
                    $('td:eq(7)', nRow).html('<input type="checkbox" checked class="WishesImages_status" id="'+mData.WishesImages.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(7)', nRow).html('<input type="checkbox" class="WishesImages_status" id="'+mData.WishesImages.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
                $('.WishesImages_status').checkboxpicker();
               
                $('.WishesImages_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'WishesImages/update_status',
                        data: {
        ...
        $('#form-field-select-1').on('change', function(e) {
            ...
            $('#DataTables_Table_0').DataTable().ajax.url("WishesImages/records?tags_id="+tags_id).load();
        });
        
    });
    
    $(function () { 
        $('#WishesImagesAddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
                "data[WishesImages][app_id]": {
                    required: true,
                },
                "data[WishesImages][title]": {
                    required: true,
                },
                "data[WishesImages][image][]": {
                    required: true,
                },
                "data[WishesImages][tags_id][]": {
                    required: true,
                },
                "data[WishesImages][views]": {
                    required: true,
                },
                "data[WishesImages][downloads]": {
                    required: true,
                },
                "data[WishesImages][modified_date]": {
                    required: true,
                },
            },
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>WishesImages/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>WishesImages/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>WishesImages/deleteAll',
                        data: {
                            id : selected_values
                        },
