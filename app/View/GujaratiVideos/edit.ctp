...
?>
<?php
    echo $this->Form->create('GujaratiVideos', array('url' => '/GujaratiVideos/save/'.$id, 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); 
?>
            ...
            <div class="col-md-9 mx-auto">
                <div class="controls">
                    ...
                    if (isset($this->request->data['GujaratiVideos']['url']) && $this->request->data['GujaratiVideos']['url'] != '') {
                        ...
                        echo "<button class='ui-button ui-widget ui-corner-all ml-1' id='confirm-delete-images' title='" . $url . "' data-id='".$this->request->data['GujaratiVideos']['id']."'><span class='ui-icon ft-image'></span>Delete GujaratiVideos</button>";
                        echo "</p>";
                    } else {
...
//                delay: 250,
    ...
    $('#GujaratiVideosEditForm').validate({
        errorElement: 'div',
    ...
    $(document).on('click', '#confirm-delete-images', function (e) {
        ...
        Swal.fire({
            ...
            if (result.value == true) {
                $.ajax({
                    type: 'POST',
                    url: '<?php echo URL_PATH; ?>GujaratiVideos/delete_image',
                    data: {
                        'id': id,
                    },
                    success: function (msg) {
                        $('#removed-p-tag').remove();
                        $('#image_loc').find('.controls').append('<input type="file" name="data[GujaratiVideos][url]" error="error" id="url1" class="form-control file"">');
