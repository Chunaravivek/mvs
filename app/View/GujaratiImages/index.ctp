            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Gujarati Images</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Gujarati Images</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('GujaratiImages', array('url' => array('controller' => 'GujaratiImages', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
                <div class="modal-body">
                    <div class="form-body">
                        ...
                        <div class="form-group row">
                            <label class="col-md-3 label-control" for="image"> Multiple GujaratiImages : </label>
                            <div class="col-md-9 mx-auto">
                                <div class="controls">
                                    <?php
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit Gujarati Images</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        $('#app_id').select2({
            ...
            width: '100%',
            ajax: {
                url: '<?php echo URL_PATH; ?>GujaratiImages/AppSelect',
                dataType: 'json',
//                delay: 250,
                data: function (data) {
                    return {
        ...
        var url = "<?php echo URL_PATH;?>GujaratiImages/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.GujaratiImages.id + '>';
                    },
                },
                {mData: 'GujaratiImages.id'},
                {mData: 'GujaratiImages.title'},
                {mData: 'GujaratiImages.tags'},
                {mData: 'GujaratiImages.image'},
                {mData: 'GujaratiImages.views'},
                {mData: 'GujaratiImages.downloads'},
                {mData: 'GujaratiImages.status','class':'project-status'},
                {mData: 'GujaratiImages.created_date'},
                {mData: 'GujaratiImages.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View GujaratiImages" onclick="ViewModal('+mData.GujaratiImages.id+')"><i class="fas fa-info"></i></a>';
//                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit GujaratiImages" onclick="EditModal('+mData.GujaratiImages.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete GujaratiImages" onclick="deleteModal('+mData.GujaratiImages.id+')"><i class="ft-trash-2"></i></a>';
                        return    delete_id;
                    },
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                $('td:eq(4)', nRow).html('<img src="'+mData.GujaratiImages.image+'" class="media-object" width="64" height="64">');
                if (mData.GujaratiImages.status == '1') {
                    $('td:eq(7)', nRow).html('<input type="checkbox" checked class="GujaratiImages_status" id="'+mData.GujaratiImages.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(7)', nRow).html('<input type="checkbox" class="GujaratiImages_status" id="'+mData.GujaratiImages.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
                $('.GujaratiImages_status').checkboxpicker();
               
                $('.GujaratiImages_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'GujaratiImages/update_status',
                        data: {
        ...
        $('#form-field-select-1').on('change', function(e) {
            ...
            $('#DataTables_Table_0').DataTable().ajax.url("GujaratiImages/records?tags_id="+tags_id).load();
        });
        
    });
    
    $(function () { 
        $('#GujaratiImagesAddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
                "data[GujaratiImages][app_id]": {
                    required: true,
                },
                "data[GujaratiImages][title]": {
                    required: true,
                },
                "data[GujaratiImages][image][]": {
                    required: true,
                },
                "data[GujaratiImages][tags_id][]": {
                    required: true,
                },
                "data[GujaratiImages][views]": {
                    required: true,
                },
                "data[GujaratiImages][downloads]": {
                    required: true,
                },
                "data[GujaratiImages][modified_date]": {
                    required: true,
                },
            },
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>GujaratiImages/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>GujaratiImages/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>GujaratiImages/deleteAll',
                        data: {
                            id : selected_values
                        },
