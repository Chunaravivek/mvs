            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Hindi Tags</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add HindiTags</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('HindiTags', array('url' => array('controller' => 'HindiTags', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit HindiTags</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        $('#tag_type').select2({
            ...
            width: '100%',
            ajax: {
                url: '<?php echo URL_PATH;?>HindiTags/TagTypeSelect',
                dataType: 'json',
//                delay: 250,
                data: function (data) {
                    return {
        ...
        var url = "<?php echo URL_PATH;?>HindiTags/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.HindiTags.id + '>';
                    },
                },
                {mData: 'HindiTags.id'},
//                {mData: 'HindiTags.tag_type',"searchable": false,"bSortable": false},
//                {mData: 'HindiTags.tag_type_text',"searchable": false,"bSortable": false},
                {mData: 'HindiTags.name'},
//                {mData: 'HindiTags.tag'},
                {mData: 'HindiTags.images'},
                {mData: 'HindiTags.order'},
                {mData: 'HindiTags.status','class':'project-status'},
                {mData: 'HindiTags.created_date'},
                {mData: 'HindiTags.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View HindiTags" onclick="ViewModal('+mData.HindiTags.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit HindiTags" onclick="EditModal('+mData.HindiTags.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete HindiTags" onclick="deleteModal('+mData.HindiTags.id+')"><i class="ft-trash-2"></i></a>';
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                $('td:eq(3)', nRow).html('<img src="'+mData.HindiTags.images+'" class="media-object" width="64" height="64">');
                if (mData.HindiTags.status == '1') {
                    $('td:eq(5)', nRow).html('<input type="checkbox" checked class="HindiTags_status" id="'+mData.HindiTags.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(5)', nRow).html('<input type="checkbox" class="HindiTags_status" id="'+mData.HindiTags.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
                $('.HindiTags_status').checkboxpicker();
               
                $('.HindiTags_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
        ...
        $('#form-field-select-1').on('change', function(e) {
            ...
            $('#DataTables_Table_0').DataTable().ajax.url("HindiTags/records?tag_type="+tag_type).load();
        });
    ...
    $(function () { 
        ...
        $('#HindiTagsAddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
                "data[HindiTags][tag_type]": {
                    required: true,
                },
                "data[HindiTags][name]": {
                    required: true,
                    remote: {
                        type: "POST",
                        url: "<?php echo URL_PATH;?>HindiTags/ifTag",
                        dataType: "json",
                        data: {
                            tag : function() {
                                return $("#name").val();
                ...
                "data[HindiTags][images]": {
                    required: true,
                },
                "data[HindiTags][order]": {
                    required: true,
                },
            },
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>HindiTags/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>HindiTags/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>HindiTags/deleteAll',
                        data: {
                            id : selected_values
                        },
