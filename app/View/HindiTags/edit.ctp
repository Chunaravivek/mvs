...
?>
<?php
    echo $this->Form->create('HindiTags', array('url' => '/HindiTags/save/'.$id, 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); 
?>
...
<!--        <div class="form-group row">
        ...
        <div class="form-group row" id="image_loc">
            ...
            <div class="col-md-9 mx-auto">
                <div class="controls">
                    <?php
                    if (isset($this->request->data['HindiTags']['images']) && $this->request->data['HindiTags']['images'] != '') {
                        echo "<p id='removed-p-tag'>";
                        echo "<img name='images' src=" .$this->request->data['HindiTags']['images'] . " height='60px' width='60px'>";
                        echo "<button class='ui-button ui-widget ui-corner-all ml-1' id='confirm-delete-images' title='" . $this->request->data['HindiTags']['images'] . "' data-id='".$this->request->data['HindiTags']['id']."'><span class='ui-icon ft-image'></span>Delete Images</button>";
                        echo "</p>";
                    } else {
            ...
            "data[HindiTags][name]": {
                ...
                remote: {
                    ...
                    data: {
                        ...
                        id : function() {
                            return <?php echo $this->request->data['HindiTags']['id']; ?>;
                        },
                    },
                    beforeSend: function() {
    ...
    $(document).on('click', '#confirm-delete-images', function (e) {
        ...
        Swal.fire({
            ...
            if (result.value == true) {
                $.ajax({
                    type: 'POST',
                    url: '<?php echo URL_PATH; ?>HindiTags/delete_image',
                    data: {
                        'id': id,
                    },
                    success: function (msg) {
                        $('#removed-p-tag').remove();
                        $('#image_loc').find('.controls').append('<input type="file" name="data[HindiTags][images]" error="error" id="images1" class="form-control file" aria-required="true">');
