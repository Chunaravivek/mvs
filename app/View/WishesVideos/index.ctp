            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Wishes Videos</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add WishesVideos</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('WishesVideos', array('url' => array('controller' => 'WishesVideos', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
...
<div class="modal animated bounceIn" id="modal-default-yt" role="dialog">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add YouTube WishesVideos </h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('WishesVideos', array('url' => array('controller' => 'WishesVideos', 'action' => 'ytadd'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
                <div class="modal-body">
                    <div class="form-body">
                        <div class="form-group row">
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit WishesVideos</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        $('#app_id').select2({
            ...
            width: '100%',
            ajax: {
                url: '<?php echo URL_PATH; ?>WishesVideos/AppSelect',
                dataType: 'json',
//                delay: 250,
                data: function (data) {
                    return {
        ...
        var url = "<?php echo URL_PATH;?>WishesVideos/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.WishesVideos.id + '>';
                    },
                },
                {mData: 'WishesVideos.id'},
                {mData: 'WishesVideos.title'},
                {mData: 'WishesVideos.tags'},
                {mData: 'WishesVideos.url'},
                {mData: 'WishesVideos.views'},
                {mData: 'WishesVideos.downloads'},
                {mData: 'WishesVideos.status','class':'project-status'},
                {mData: 'WishesVideos.created_date'},
                {mData: 'WishesVideos.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View WishesVideos" onclick="ViewModal('+mData.WishesVideos.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit WishesVideos" onclick="EditModal('+mData.WishesVideos.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete WishesVideos" onclick="deleteModal('+mData.WishesVideos.id+')"><i class="ft-trash-2"></i></a>';
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                var thumb = mData.WishesVideos.url  + '.jpg';
//                $('td:eq(4)', nRow).html('<video controls poster="' + thumb + '" style="height: 200px;width: 350px"><source src="' + mData.WishesVideos.url + '" type="video/mp4"></video>');
                $('td:eq(4)', nRow).html('<a class="ui-button ui-widget ui-corner-all mb-2" href="' + mData.WishesVideos.url + '" target="_blank"><span class="ui-icon ft-play"></span></a>');
                if (mData.WishesVideos.status == '1') {
                    $('td:eq(7)', nRow).html('<input type="checkbox" checked class="WishesVideos_status" id="'+mData.WishesVideos.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(7)', nRow).html('<input type="checkbox" class="WishesVideos_status" id="'+mData.WishesVideos.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
                $('.WishesVideos_status').checkboxpicker();
               
                $('.WishesVideos_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'WishesVideos/update_status',
                        data: {
        ...
        $('#form-field-select-1').on('change', function(e) {
            ...
            $('#DataTables_Table_0').DataTable().ajax.url("WishesVideos/records?tags_id="+tags_id).load();
        });
        
    });
    
    $(function () { 
        $('#WishesVideosAddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
                "data[WishesVideos][app_id]": {
                    required: true,
                },
                "data[WishesVideos][yturl]": {
                    required: true,
                    url: true,
                },
                "data[WishesVideos][title]": {
                    required: true,
                },
                "data[WishesVideos][url]": {
                    required: true,
                },
                "data[WishesVideos][tags_id][]": {
                    required: true,
                },
                "data[WishesVideos][views]": {
                    required: true,
                },
                "data[WishesVideos][downloads]": {
                    required: true,
                },
                "data[WishesVideos][modified_date]": {
                    required: true,
                },
            },
        ...
        $('#WishesVideosYtaddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>WishesVideos/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>WishesVideos/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>WishesVideos/deleteAll',
                        data: {
                            id : selected_values
                        },
