            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Punjabi Makers</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Punjabi Makers</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('PunjabiMakers', array('url' => array('controller' => 'PunjabiMakers', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
...
<div class="modal animated bounceIn" id="modal-default-yt" role="dialog">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add YouTube PunjabiMakers </h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('PunjabiMakers', array('url' => array('controller' => 'PunjabiMakers', 'action' => 'ytadd'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
                <div class="modal-body">
                    <div class="form-body">
                        <div class="form-group row">
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit PunjabiMakers</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        $('#app_id').select2({
            ...
            width: '100%',
            ajax: {
                url: '<?php echo URL_PATH; ?>PunjabiMakers/AppSelect',
                dataType: 'json',
//                delay: 250,
                data: function (data) {
                    return {
        ...
        var url = "<?php echo URL_PATH;?>PunjabiMakers/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.PunjabiMakers.id + '>';
                    },
                },
                {mData: 'PunjabiMakers.id'},
                {mData: 'PunjabiMakers.tags'},
                {mData: 'PunjabiMakers.title'},
                {mData: 'PunjabiMakers.url'},
                {mData: 'PunjabiMakers.image'},
                {mData: 'PunjabiMakers.zip'},
                {mData: 'PunjabiMakers.views'},
                {mData: 'PunjabiMakers.downloads'},
                {mData: 'PunjabiMakers.status','class':'project-status'},
                {mData: 'PunjabiMakers.created_date'},
                {mData: 'PunjabiMakers.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View PunjabiMakers" onclick="ViewModal('+mData.PunjabiMakers.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit PunjabiMakers" onclick="EditModal('+mData.PunjabiMakers.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete PunjabiMakers" onclick="deleteModal('+mData.PunjabiMakers.id+')"><i class="ft-trash-2"></i></a>';
                        return   delete_id;
                    },
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                var thumb = mData.PunjabiMakers.url  + '.jpg';
                $('td:eq(4)', nRow).html('<video controls poster="' + mData.PunjabiMakers.image + '" style="height: 200px;width: 350px"><source src="' + mData.PunjabiMakers.url + '" type="video/mp4"></video>');
//                $('td:eq(4)', nRow).html('<a class="ui-button ui-widget ui-corner-all mb-2" href="' + mData.PunjabiMakers.url + '" target="_blank"><span class="ui-icon ft-play"></span></a>');
                $('td:eq(5)', nRow).html('<a class="ui-button ui-widget ui-corner-all mb-2" href="' + mData.PunjabiMakers.image + '" target="_blank"><span class="ui-icon ft-camera"></span></a>');
                $('td:eq(6)', nRow).html(mData.PunjabiMakers.zip);
                if (mData.PunjabiMakers.status == '1') {
                    $('td:eq(9)', nRow).html('<input type="checkbox" checked class="PunjabiMakers_status" id="'+mData.PunjabiMakers.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(9)', nRow).html('<input type="checkbox" class="PunjabiMakers_status" id="'+mData.PunjabiMakers.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
                $('.PunjabiMakers_status').checkboxpicker();
               
                $('.PunjabiMakers_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'PunjabiMakers/update_status',
                        data: {
        ...
        $('#form-field-select-1').on('change', function(e) {
            ...
            $('#DataTables_Table_0').DataTable().ajax.url("PunjabiMakers/records?tags_id="+tags_id).load();
        });
        
    });
    
    $(function () { 
        $('#PunjabiMakersAddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
                "data[PunjabiMakers][app_id]": {
                    required: true,
                },
                "data[PunjabiMakers][yturl]": {
                    required: true,
                    url: true,
                },
                "data[PunjabiMakers][title]": {
                    required: true,
                },
                "data[PunjabiMakers][url]": {
                    required: true,
                },
                "data[PunjabiMakers][tags_id]": {
                    required: true,
                },
                "data[PunjabiMakers][views]": {
                    required: true,
                },
                "data[PunjabiMakers][downloads]": {
                    required: true,
                },
                "data[PunjabiMakers][modified_date]": {
                    required: true,
                },
            },
        ...
        $('#PunjabiMakersYtaddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>PunjabiMakers/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>PunjabiMakers/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>PunjabiMakers/deleteAll',
                        data: {
                            id : selected_values
                        },
