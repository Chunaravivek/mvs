    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Applications</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('Applications', array('url' => array('controller' => 'Applications', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit Applications</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
//                delay: 250,
        ...
        var url = "<?php echo URL_PATH; ?>Applications/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.Applications.id + '>';
                    },
                },
                {mData: 'Applications.id'},
                {mData: 'Applications.name'},
                {mData: 'Applications.account_name', "searchable": false, "orderable": false, "bSortable": false, },
                {mData: 'Applications.app_code'},
                {mData: 'Applications.package_name'},
                {mData: 'Applications.app_version'},
                {mData: 'Applications.status', 'class': 'project-status'},
                {mData: 'Applications.created_date'},
                {mData: 'Applications.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function (mData, type, full) {
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View Applications" onclick="ViewModal('+mData.Applications.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit Applications" onclick="EditModal(' + mData.Applications.id + ')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete Applications" onclick="deleteModal(' + mData.Applications.id + ')"><i class="ft-trash-2"></i></a>';
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function (nRow, mData, iDataIndex) {
                if (mData.Applications.status == '1') {
                    $('td:eq(6)', nRow).html('<input type="checkbox" checked class="Applications_status" id="' + mData.Applications.id + '" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(6)', nRow).html('<input type="checkbox" class="Applications_status" id="' + mData.Applications.id + '" data-group-cls="btn-group-sm" hidden="">');
                }

            },
            "fnDrawCallback": function () {
                $('.Applications_status').checkboxpicker();

                $('.Applications_status:checkbox').checkboxpicker().on('change', function () {
                    var id = $(this).attr('id');
...
//                    },
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>Applications/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data: {id: id},
            url: "<?php echo URL_PATH; ?>Applications/edit",
            cache: false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>Applications/deleteAll',
                        data: {
                            id : selected_values
                        },
