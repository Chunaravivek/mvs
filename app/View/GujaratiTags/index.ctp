            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Gujarati Tags</h4>
...
<!--                                    <label>Gujarati Tag Types Name</label>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add GujaratiTags</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('GujaratiTags', array('url' => array('controller' => 'GujaratiTags', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit GujaratiTags</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        $('#tag_type').select2({
            ...
            width: '100%',
            ajax: {
                url: '<?php echo URL_PATH;?>GujaratiTags/TagTypeSelect',
                dataType: 'json',
//                delay: 250,
                data: function (data) {
                    return {
        ...
        var url = "<?php echo URL_PATH;?>GujaratiTags/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.GujaratiTags.id + '>';
                    },
                },
                {mData: 'GujaratiTags.id'},
//                {mData: 'GujaratiTags.tag_type',"searchable": false,"bSortable": false},
//                {mData: 'GujaratiTags.tag_type_text',"searchable": false,"bSortable": false},
                {mData: 'GujaratiTags.name'},
//                {mData: 'GujaratiTags.tag'},
                {mData: 'GujaratiTags.images'},
                {mData: 'GujaratiTags.order'},
                {mData: 'GujaratiTags.status','class':'project-status'},
                {mData: 'GujaratiTags.created_date'},
                {mData: 'GujaratiTags.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View GujaratiTags" onclick="ViewModal('+mData.GujaratiTags.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit GujaratiTags" onclick="EditModal('+mData.GujaratiTags.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete GujaratiTags" onclick="deleteModal('+mData.GujaratiTags.id+')"><i class="ft-trash-2"></i></a>';
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                $('td:eq(3)', nRow).html('<img src="'+mData.GujaratiTags.images+'" class="media-object" width="64" height="64">');
                if (mData.GujaratiTags.status == '1') {
                    $('td:eq(5)', nRow).html('<input type="checkbox" checked class="GujaratiTags_status" id="'+mData.GujaratiTags.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(5)', nRow).html('<input type="checkbox" class="GujaratiTags_status" id="'+mData.GujaratiTags.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
                $('.GujaratiTags_status').checkboxpicker();
               
                $('.GujaratiTags_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'GujaratiTags/update_status',
                        data: {
        ...
        $('#form-field-select-1').on('change', function(e) {
            ...
            $('#DataTables_Table_0').DataTable().ajax.url("GujaratiTags/records?tag_type="+tag_type).load();
        });
    ...
    $(function () { 
        ...
        $('#GujaratiTagsAddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
                "data[GujaratiTags][tag_type]": {
                    required: true,
                },
                "data[GujaratiTags][name]": {
                    required: true,
                    remote: {
                        type: "POST",
                        url: "<?php echo URL_PATH;?>GujaratiTags/ifTag",
                        dataType: "json",
                        data: {
                            tag : function() {
                                return $("#name").val();
                ...
                "data[GujaratiTags][images]": {
                    required: true,
                },
                "data[GujaratiTags][order]": {
                    required: true,
                },
            },
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>GujaratiTags/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>GujaratiTags/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>GujaratiTags/deleteAll',
                        data: {
                            id : selected_values
                        },
