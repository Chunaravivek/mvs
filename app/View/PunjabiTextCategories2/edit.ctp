...
?>
<?php
    echo $this->Form->create('PunjabiTextCategories2', array('url' => '/PunjabiTextCategories2/save/'.$id, 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); 
?>
<div class="modal-body">
    <div class="form-body">
        ...
        <div class="form-group row color">
            <label class="col-md-2 label-control" for="color1"> Color : </label>
            <div class="col-md-9 mx-auto">
                <div class="controls">
                    <?php
        ...
        <div class="form-group row" id="image_loc">
            ...
            <div class="col-md-9 mx-auto">
                <div class="controls">
                    <?php
                    if (isset($this->request->data['PunjabiTextCategories2']['image']) && $this->request->data['PunjabiTextCategories2']['image'] != '') {
                        echo "<p id='removed-p-tag'>";
                        echo "<img name='image' src=" .$this->request->data['PunjabiTextCategories2']['image'] . " height='60px' width='60px'>";
                        echo "<button class='ui-button ui-widget ui-corner-all ml-1' id='confirm-delete-image' title='" . $this->request->data['PunjabiTextCategories2']['image'] . "' data-id='".$this->request->data['PunjabiTextCategories2']['id']."'><span class='ui-icon ft-image'></span>Delete Image</button>";
                        echo "</p>";
                    } else {
...
$(document).ready(function(){
    ...
    $('#PunjabiTextCategories2EditForm').validate({
        errorElement: 'div',
    ...
    $(document).on('click', '#confirm-delete-image', function (e) {
        ...
        Swal.fire({
            ...
            if (result.value == true) {
                $.ajax({
                    type: 'POST',
                    url: '<?php echo URL_PATH; ?>PunjabiTextCategories2/delete_image',
                    data: {
                        'id': id,
                    },
                    success: function (msg) {
                        $('#removed-p-tag').remove();
                        $('#image_loc').find('.controls').append('<input type="file" name="data[PunjabiTextCategories2][image]" error="error" id="image1" class="form-control file" aria-required="true">');
