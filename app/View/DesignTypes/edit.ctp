...
?>
<?php
    echo $this->Form->create('DesignTypes', array('url' => '/DesignTypes/save/'.$id, 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); 
?>
    ...
    <div class="form-body">
        ...
        <div class="form-group row" id="image_loc">
            ...
            <div class="col-md-9 mx-auto">
                <div class="controls">
                    <?php
                    if (isset($this->request->data['DesignTypes']['image']) && $this->request->data['DesignTypes']['image'] != '') {
                        echo "<p id='removed-p-tag'>";
                        echo "<img name='image' src=" .$this->request->data['DesignTypes']['image'] . " height='60px' width='60px'>";
                        echo "<button class='ui-button ui-widget ui-corner-all ml-1' id='confirm-delete-image' title='" . $this->request->data['DesignTypes']['image'] . "' data-id='".$this->request->data['DesignTypes']['id']."'><span class='ui-icon ft-image'></span>Delete Image</button>";
                        echo "</p>";
                    } else {
    ...
    $(document).on('click', '#confirm-delete-image', function (e) {
        ...
        Swal.fire({
            ...
            if (result.value == true) {
                $.ajax({
                    type: 'POST',
                    url: '<?php echo URL_PATH; ?>DesignTypes/delete_image',
                    data: {
                        'id': id,
                    },
                    success: function (msg) {
                        $('#removed-p-tag').remove();
                        $('#image_loc').find('.controls').append('<input type="file" name="data[DesignTypes][image]" error="error" id="image1" class="form-control file" aria-required="true">');
