            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Design Types</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Design Types</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('DesignTypes', array('url' => array('controller' => 'DesignTypes', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit Design Types</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        ...
        var url = "<?php echo URL_PATH;?>DesignTypes/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.DesignTypes.id + '>';
                    },
                },
                {mData: 'DesignTypes.id'},
                {mData: 'DesignTypes.name'},
                {mData: 'DesignTypes.image'},
                {mData: 'DesignTypes.status','class':'project-status'},
                {mData: 'DesignTypes.created_date'},
                {mData: 'DesignTypes.modified_date'},
                {
                    "targets": 8,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View DesignTypes" onclick="ViewModal('+mData.DesignTypes.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit DesignTypes" onclick="EditModal('+mData.DesignTypes.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete DesignTypes" onclick="deleteModal('+mData.DesignTypes.id+')"><i class="ft-trash-2"></i></a>';
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                $('td:eq(3)', nRow).html('<img src="'+mData.DesignTypes.image+'" class="media-object" width="64" height="64">');
                if (mData.DesignTypes.status == '1') {
                    $('td:eq(4)', nRow).html('<input type="checkbox" checked class="DesignTypes_status" id="'+mData.DesignTypes.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(4)', nRow).html('<input type="checkbox" class="DesignTypes_status" id="'+mData.DesignTypes.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
               $('.DesignTypes_status').checkboxpicker();
               
                $('.DesignTypes_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
    ...
    $(function () { 
        $('#DesignTypesAddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
                "data[DesignTypes][name]": {
                    required: true,
                },
                "data[DesignTypes][image]": {
                    required: true,
                },
            },
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>DesignTypes/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    $(function() {
        $(document).on('click', '#confirmOk', function () {
                ...
                var this_delete = $("#confirmCancel");
                this_delete.remove();
               window.location.href= "<?php echo $this->webroot; ?>DesignTypes/delete/"+userId;
        });
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>DesignTypes/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>DesignTypes/deleteAll',
                        data: {
                            id : selected_values
                        },
