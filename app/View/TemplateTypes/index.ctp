            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Template Types</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Template Types</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('TemplateTypes', array('url' => array('controller' => 'TemplateTypes', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit Template Types</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        ...
        var url = "<?php echo URL_PATH;?>TemplateTypes/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.TemplateTypes.id + '>';
                    },
                },
                {mData: 'TemplateTypes.id'},
                {mData: 'TemplateTypes.type'},
                {mData: 'TemplateTypes.status','class':'project-status'},
                {mData: 'TemplateTypes.created_date'},
                {mData: 'TemplateTypes.modified_date'},
                {
                    "targets": 8,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View TemplateTypes" onclick="ViewModal('+mData.TemplateTypes.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit TemplateTypes" onclick="EditModal('+mData.TemplateTypes.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete TemplateTypes" onclick="deleteModal('+mData.TemplateTypes.id+')"><i class="ft-trash-2"></i></a>';
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                if (mData.TemplateTypes.status == '1') {
                    $('td:eq(3)', nRow).html('<input type="checkbox" checked class="TemplateTypes_status" id="'+mData.TemplateTypes.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(3)', nRow).html('<input type="checkbox" class="TemplateTypes_status" id="'+mData.TemplateTypes.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
               $('.TemplateTypes_status').checkboxpicker();
               
                $('.TemplateTypes_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'TemplateTypes/update_status',
                        data: {
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>TemplateTypes/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    $(function() {
        $(document).on('click', '#confirmOk', function () {
                ...
                var this_delete = $("#confirmCancel");
                this_delete.remove();
               window.location.href= "<?php echo $this->webroot; ?>TemplateTypes/delete/"+userId;
        });
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>TemplateTypes/deleteAll',
                        data: {
                            id : selected_values
                        },
...
//                            window.location = response;
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>TemplateTypes/edit",
            cache:false,
