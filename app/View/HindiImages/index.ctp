            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Hindi Images</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Hindi Images</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('HindiImages', array('url' => array('controller' => 'HindiImages', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
                <div class="modal-body">
                    <div class="form-body">
                        ...
                        <div class="form-group row">
                            <label class="col-md-3 label-control" for="image"> Multiple HindiImages : </label>
                            <div class="col-md-9 mx-auto">
                                <div class="controls">
                                    <?php
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit Hindi Images</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        $('#app_id').select2({
            ...
            width: '100%',
            ajax: {
                url: '<?php echo URL_PATH; ?>HindiImages/AppSelect',
                dataType: 'json',
//                delay: 250,
                data: function (data) {
                    return {
        ...
        var url = "<?php echo URL_PATH;?>HindiImages/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.HindiImages.id + '>';
                    },
                },
                {mData: 'HindiImages.id'},
                {mData: 'HindiImages.title'},
                {mData: 'HindiImages.tags'},
                {mData: 'HindiImages.image'},
                {mData: 'HindiImages.views'},
                {mData: 'HindiImages.downloads'},
                {mData: 'HindiImages.status','class':'project-status'},
                {mData: 'HindiImages.created_date'},
                {mData: 'HindiImages.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View HindiImages" onclick="ViewModal('+mData.HindiImages.id+')"><i class="fas fa-info"></i></a>';
//                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit HindiImages" onclick="EditModal('+mData.HindiImages.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete HindiImages" onclick="deleteModal('+mData.HindiImages.id+')"><i class="ft-trash-2"></i></a>';
                        return    delete_id;
                    },
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                $('td:eq(4)', nRow).html('<img src="'+mData.HindiImages.image+'" class="media-object" width="64" height="64">');
                if (mData.HindiImages.status == '1') {
                    $('td:eq(7)', nRow).html('<input type="checkbox" checked class="HindiImages_status" id="'+mData.HindiImages.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(7)', nRow).html('<input type="checkbox" class="HindiImages_status" id="'+mData.HindiImages.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
                $('.HindiImages_status').checkboxpicker();
               
                $('.HindiImages_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
        ...
        $('#form-field-select-1').on('change', function(e) {
            ...
            $('#DataTables_Table_0').DataTable().ajax.url("HindiImages/records?tags_id="+tags_id).load();
        });
        
    });
    
    $(function () { 
        $('#HindiImagesAddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
                "data[HindiImages][app_id]": {
                    required: true,
                },
                "data[HindiImages][title]": {
                    required: true,
                },
                "data[HindiImages][image][]": {
                    required: true,
                },
                "data[HindiImages][tags_id][]": {
                    required: true,
                },
                "data[HindiImages][views]": {
                    required: true,
                },
                "data[HindiImages][downloads]": {
                    required: true,
                },
                "data[HindiImages][modified_date]": {
                    required: true,
                },
            },
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>HindiImages/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>HindiImages/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>HindiImages/deleteAll',
                        data: {
                            id : selected_values
                        },
