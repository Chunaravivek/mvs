            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Punjabi Tags</h4>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-sm-6 col-md-4 col-lg-4 col-xl-3 col-xxl-2">
                                <div class="form-group">
                                    <!--<label>Punjabi Tag Types Name</label>-->
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add PunjabiTags</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('PunjabiTags', array('url' => array('controller' => 'PunjabiTags', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit PunjabiTags</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        $('#tag_type').select2({
            ...
            width: '100%',
            ajax: {
                url: '<?php echo URL_PATH;?>PunjabiTags/TagTypeSelect',
                dataType: 'json',
//                delay: 250,
                data: function (data) {
                    return {
        ...
        var url = "<?php echo URL_PATH;?>PunjabiTags/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.PunjabiTags.id + '>';
                    },
                },
                {mData: 'PunjabiTags.id'},
//                {mData: 'PunjabiTags.tag_type',"searchable": false,"bSortable": false},
//                {mData: 'PunjabiTags.tag_type_text',"searchable": false,"bSortable": false},
                {mData: 'PunjabiTags.name'},
                {mData: 'PunjabiTags.images'},
                {mData: 'PunjabiTags.order'},
                {mData: 'PunjabiTags.status','class':'project-status'},
                {mData: 'PunjabiTags.created_date'},
                {mData: 'PunjabiTags.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
...
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View PunjabiTags" onclick="ViewModal('+mData.PunjabiTags.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit PunjabiTags" onclick="EditModal('+mData.PunjabiTags.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete PunjabiTags" onclick="deleteModal('+mData.PunjabiTags.id+')"><i class="ft-trash-2"></i></a>';
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                $('td:eq(3)', nRow).html('<img src="'+mData.PunjabiTags.images+'" class="media-object" width="64" height="64">');
                if (mData.PunjabiTags.status == '1') {
                    $('td:eq(5)', nRow).html('<input type="checkbox" checked class="PunjabiTags_status" id="'+mData.PunjabiTags.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(5)', nRow).html('<input type="checkbox" class="PunjabiTags_status" id="'+mData.PunjabiTags.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
                $('.PunjabiTags_status').checkboxpicker();
               
                $('.PunjabiTags_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
        ...
        $('#form-field-select-1').on('change', function(e) {
            ...
            $('#DataTables_Table_0').DataTable().ajax.url("PunjabiTags/records?tag_type="+tag_type).load();
        });
    ...
    $(function () { 
        ...
        $('#PunjabiTagsAddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
                "data[PunjabiTags][tag_type]": {
                    required: true,
                },
                "data[PunjabiTags][name]": {
                    required: true,
                    remote: {
                        type: "POST",
                        url: "<?php echo URL_PATH;?>PunjabiTags/ifTag",
                        dataType: "json",
                        data: {
                            tag : function() {
                                return $("#name").val();
                ...
                "data[PunjabiTags][images]": {
                    required: true,
                },
                "data[PunjabiTags][order]": {
                    required: true,
                },
            },
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>PunjabiTags/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>PunjabiTags/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>PunjabiTags/deleteAll',
                        data: {
                            id : selected_values
                        },
