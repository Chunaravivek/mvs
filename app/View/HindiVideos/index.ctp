            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Hindi Videos</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add HindiVideos</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('HindiVideos', array('url' => array('controller' => 'HindiVideos', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
...
<div class="modal animated bounceIn" id="modal-default-yt" role="dialog">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add YouTube HindiVideos </h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('HindiVideos', array('url' => array('controller' => 'HindiVideos', 'action' => 'ytadd'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
                <div class="modal-body">
                    <div class="form-body">
                        <div class="form-group row">
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit HindiVideos</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
//                delay: 250,
        ...
        var url = "<?php echo URL_PATH;?>HindiVideos/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.HindiVideos.id + '>';
                    },
                },
                {mData: 'HindiVideos.id'},
                {mData: 'HindiVideos.title'},
                {mData: 'HindiVideos.tags'},
                {mData: 'HindiVideos.url'},
                {mData: 'HindiVideos.views'},
                {mData: 'HindiVideos.downloads'},
                {mData: 'HindiVideos.status','class':'project-status'},
                {mData: 'HindiVideos.created_date'},
                {mData: 'HindiVideos.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View HindiVideos" onclick="ViewModal('+mData.HindiVideos.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit HindiVideos" onclick="EditModal('+mData.HindiVideos.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete HindiVideos" onclick="deleteModal('+mData.HindiVideos.id+')"><i class="ft-trash-2"></i></a>';
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                var thumb = mData.HindiVideos.url  + '.jpg';
//                $('td:eq(4)', nRow).html('<video controls poster="' + thumb + '" style="height: 200px;width: 350px"><source src="' + mData.HindiVideos.url + '" type="video/mp4"></video>');
                $('td:eq(4)', nRow).html('<a class="ui-button ui-widget ui-corner-all mb-2" href="' + mData.HindiVideos.url + '" target="_blank"><span class="ui-icon ft-play"></span></a>');
                if (mData.HindiVideos.status == '1') {
                    $('td:eq(7)', nRow).html('<input type="checkbox" checked class="HindiVideos_status" id="'+mData.HindiVideos.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(7)', nRow).html('<input type="checkbox" class="HindiVideos_status" id="'+mData.HindiVideos.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
                $('.HindiVideos_status').checkboxpicker();
               
                $('.HindiVideos_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
        ...
        $('#form-field-select-1').on('change', function(e) {
            ...
            $('#DataTables_Table_0').DataTable().ajax.url("HindiVideos/records?tags_id="+tags_id).load();
        });
        
    });
    
    $(function () { 
        $('#HindiVideosAddForm').validate({
            errorElement: 'div',
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>HindiVideos/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>HindiVideos/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>HindiVideos/deleteAll',
                        data: {
                            id : selected_values
                        },
