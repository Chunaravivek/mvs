            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage God Text Categories</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add GodTextCategories</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('GodTextCategories', array('url' => array('controller' => 'GodTextCategories', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit God Text Categories</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        ...
        var url = "<?php echo URL_PATH;?>GodTextCategories/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.GodTextCategories.id + '>';
                    },
                },
                {mData: 'GodTextCategories.id'},
                {mData: 'GodTextCategories.cat_name'},
                {mData: 'GodTextCategories.image'},
                {mData: 'GodTextCategories.order'},
                {mData: 'GodTextCategories.status','class':'project-status'},
                {mData: 'GodTextCategories.created_date'},
                {mData: 'GodTextCategories.modified_date'},
                {
                    "targets": -1,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View GodTextCategories" onclick="ViewModal('+mData.GodTextCategories.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit GodTextCategories" onclick="EditModal('+mData.GodTextCategories.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" data-backdrop="static" data-keyboard="false" title="Delete GodTextCategories" onclick="deleteModal('+mData.GodTextCategories.id+')"><i class="ft-trash-2"></i></a>';
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                $('td:eq(3)', nRow).html('<img src="'+mData.GodTextCategories.image+'" class="media-object" width="64" height="64">');
                if (mData.GodTextCategories.status == '1') {
                    $('td:eq(5)', nRow).html('<input type="checkbox" checked class="GodTextCategories_status" id="'+mData.GodTextCategories.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(5)', nRow).html('<input type="checkbox" class="GodTextCategories_status" id="'+mData.GodTextCategories.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
               $('.GodTextCategories_status').checkboxpicker();
               
                $('.GodTextCategories_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'GodTextCategories/update_status',
                        data: {
    ...
    $(function () { 
        $('#GodTextCategoriesAddForm').validate({
            errorElement: 'div',
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>GodTextCategories/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>GodTextCategories/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>GodTextCategories/deleteAll',
                        data: {
                            id : selected_values
                        },
