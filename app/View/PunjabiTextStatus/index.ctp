            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Manage Punjabi Text Status</h4>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add PunjabiTextStatus</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <?php echo $this->Form->create('PunjabiTextStatus', array('url' => array('controller' => 'PunjabiTextStatus', 'action' => 'add'), 'class' => 'form form-horizontal', 'enctype' => 'multipart/form-data')); ?>
...
<div class="modal animated bounceIn bd-example-modal-lg" id="edit" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Edit PunjabiTextStatus</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script type="text/javascript" language="javascript" class="init">
    $(document).ready( function () {
        $('.tag_id').select2({
            ...
            width: '100%',
            ajax: {
//                url: '<?php echo URL_PATH; ?>PunjabiTextStatus/TextCategorieSelect',
                url: '<?php echo URL_PATH; ?>PunjabiTextStatus/TagSelect',
                dataType: 'json',
//                delay: 250,
                data: function (data) {
                    return {
        ...
        var url = "<?php echo URL_PATH;?>PunjabiTextStatus/records.json";
            ...
            aoColumns: [
                {
                    ...
                    "mRender": function (mData, type, full) {
                        return  '<input type="checkbox" id="checkall" class="video_checkbox" name="mydata" data-video-id=' + mData.PunjabiTextStatus.id + '>';
                    },
                },
                {mData: 'PunjabiTextStatus.id'},
                {mData: 'PunjabiTextStatus.tag_name'},
                {mData: 'PunjabiTextStatus.text'},
                {mData: 'PunjabiTextStatus.status','class':'project-status'},
                {mData: 'PunjabiTextStatus.created_date'},
                {mData: 'PunjabiTextStatus.modified_date'},
                {
                    "targets": 5,
                    "searchable": false,
                    "data": null,
                    ...
                    "orderable": false,
                    "mRender": function(mData, type, full) {  
//                        var View = '<a href="#" class="btn btn-sm btn-app bg-info" data-backdrop="static" data-keyboard="false" title="View PunjabiTextStatus" onclick="ViewModal('+mData.PunjabiTextStatus.id+')"><i class="fas fa-info"></i></a>';
                        var edit = '<a href="javascript:void(0);" class="btn btn-primary btn-sm mr-2" data-target="#modal-edit" data-backdrop="static" data-keyboard="false" title="Edit PunjabiTextStatus" onclick="EditModal('+mData.PunjabiTextStatus.id+')"><i class="ft-edit-1"></i></a>';
                        var delete_id = '<a href="javascript:void(0);" class="btn btn-danger btn-sm" title="Delete PunjabiTextStatus" onclick="deleteModal('+mData.PunjabiTextStatus.id+')"><i class="ft-trash-2"></i></a>';
...
//                $('td', nRow).eq(0).html(iDisplayIndex + 1);
            ...
            "fnCreatedRow": function(nRow, mData, iDataIndex){ 
                if (mData.PunjabiTextStatus.status == '1') {
                    $('td:eq(4)', nRow).html('<input type="checkbox" checked class="PunjabiTextStatus_status" id="'+mData.PunjabiTextStatus.id+'" data-group-cls="btn-group-sm" hidden="">');
                } else {
                    $('td:eq(4)', nRow).html('<input type="checkbox" class="PunjabiTextStatus_status" id="'+mData.PunjabiTextStatus.id+'" data-group-cls="btn-group-sm" hidden="">');
                }
             
            },
            "fnDrawCallback": function() {
                $('.PunjabiTextStatus_status').checkboxpicker();
               
                $('.PunjabiTextStatus_status:checkbox').checkboxpicker().on('change', function() {
                    var id = $(this).attr('id');
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'PunjabiTextStatus/update_status',
                        data: {
    ...
    $(function () { 
        $('#PunjabiTextStatusAddForm').validate({
            errorElement: 'div',
            errorClass: 'help-block',
            focusInvalid: false,
            rules: {
                "data[PunjabiTextStatus][tag_id]": {
                    required: true,
                },
                "data[PunjabiTextStatus][text]": {
                    required: true,
                },
            },
        ...
        $('#form-field-select-1').on('change', function(e) {
            ...
            $('#DataTables_Table_0').DataTable().ajax.url("PunjabiTextStatus/records?tags_id="+tags_id).load();
        });
    });
            ...
            buttons :  { 
                "MyButton" : {
                    ...
                    click: function(){
                        ...
                        window.location.href= "<?php echo $this->webroot; ?>PunjabiTextStatus/delete/"+id;
                    }   
                }, 
                "MyButton1" : {
                    text: "Cancel",
    ...
    function EditModal(id) {
        ...
        $.ajax({
            ...
            dataType: '',
            data:{id:id}, 
            url:"<?php echo URL_PATH;?>PunjabiTextStatus/edit",
            cache:false,
        ...
        if (user.length <= 0) {
            ...
            cancelButtonText: "No, cancel plx!",
                ...
                if (result.value == true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo URL_PATH; ?>PunjabiTextStatus/deleteAll',
                        data: {
                            id : selected_values
                        },
